    ...
    if t~=24*60
        ...
        if t~=24*60
            ...
            if t_leave1-t_home>=60+t_charge
                ...
                SOC_alter(1,:)=FUNC_alter1(SOC,t_leave0,t_home,t_leave1,t_charge);
                SOC_alter(2,:)=FUNC_altPlan2(SOC,t_leave0,t_home,t_leave1,t_charge);
                SOC_alter(3,:)=FUNC_altPlan3(SOC,t_leave0,t_home,t_leave1,t_charge);
                SOC_alter(4,:)=FUNC_altPlan4(SOC,t_leave0,t_home,t_leave1,t_charge);
                SOC_alter(5,:)=FUNC_altPlan5(SOC,t_leave0,t_home,t_leave1,t_charge);
            end
        elseif t_leave0+24*60-t_home>=60+t_charge
            %apply alternative plans
            SOC_alter(1,:)=FUNC_alter1(SOC,t_leave0,t_home,t_leave0+24*60,t_charge);
            SOC_alter(2,:)=FUNC_altPlan2(SOC,t_leave0,t_home,t_leave0+24*60,t_charge);
            SOC_alter(3,:)=FUNC_altPlan3(SOC,t_leave0,t_home,t_leave0+24*60,t_charge);
            SOC_alter(4,:)=FUNC_altPlan4(SOC,t_leave0,t_home,t_leave0+24*60,t_charge);
            SOC_alter(5,:)=FUNC_altPlan5(SOC,t_leave0,t_home,t_leave0+24*60,t_charge);
        end
    end
end
...
function [ SOC_a1 ] =FUNC_alter1 ( SOC, t_leave0, t_home, t_leave, t_charge)
    %alternative plan 1
    SOC_2day=[SOC SOC];
...
function [ altSOC2_2day ] = altPlan2(SOC, t_leave0, t_home, t_leave, t_charge)
...
% This plan charges a vehicle halfway as soon as the car is parked, pauses, and charges up to 100% at the end of the parking hours 
    ...
    for i = (t_home + 1):t_charge_half
        ...
        altSOC2_2day(i) = altSOC2_2day(t_home) + (chargeKW/capacityKWh)*(100/60)*(i-t_home);
    end
    ...
    for i = (t_charge_half + pause + 1):(t_leave - 60)
        altSOC2_2day(i) = altSOC2_2day(t_charge_half + pause) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_half + pause));
...
function [ altSOC3 ] = altPlan3(SOC, t_leave0, t_home, t_leave, t_charge)
...
% This plan charges a vehicle halfway as soon as the car is parked, takes a break of random length, and charges up to 100% 
    ...
    for i = (t_home + 1):t_charge_half
        % SOC increases by (chargeKW/capacityKWh)*(100/60) every minute
        altSOC3_2day(i) = altSOC3_2day(t_home) + (chargeKW/capacityKWh)*(100/60)*(i-t_home);
    end
    ...
    for i = (t_charge_half + pause + 1):(t_charge_half + pause + t_charge/2)
        altSOC3_2day(i) = altSOC3_2day(t_charge_half + pause) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_half + pause));
...
function [ altSOC4 ] = altPlan4(SOC, t_leave0, t_home, t_leave, t_charge)
...
% This plan charges a vehicle in three discrete steps that are evenly spaced
    ...
    for i = (t_home + 1):t_charge_step1
        % SOC increases by (chargeKW/capacityKWh)*(100/60) every minute
        altSOC4_2day(i) = altSOC4_2day(t_home) + (chargeKW/capacityKWh)*(100/60)*(i-t_home);
    end
    ...
    for i = (t_charge_step1 + pause + 1):(t_charge_step2)
        altSOC4_2day(i) = altSOC4_2day(t_charge_step1 + pause) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_step1 + pause));
    end
    ...
    for i = (t_charge_step2 + pause + 1):(t_leave - 60)
        altSOC4_2day(i) = altSOC4_2day(t_charge_step2 + pause) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_step2 + pause));
...
function [ altSOC5 ] = altPlan5(SOC,t_leave0, t_home, t_leave, t_charge)
% This subfunction finds the SOC when the fourth alternative plan is applied
% This plan charges a vehicle in three discrete steps; breaks among the charging steps are randomly determined 
    ...
    for i = (t_home + 1):t_charge_step1
        % SOC increases by (chargeKW/capacityKWh)*(100/60) every minute
        altSOC5_2day(i) = altSOC5_2day(t_home) + (chargeKW/capacityKWh)*(100/60)*(i-t_home);
    end
    ...
    for i = (t_charge_step1 + pause1 + 1):(t_charge_step2)
        altSOC5_2day(i) = altSOC5_2day(t_charge_step1 + pause1) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_step1 + pause1));
    end
    ...
    for i = (t_charge_step2 + pause2 + 1):(t_charge_step3)
        altSOC5_2day(i) = altSOC5_2day(t_charge_step2 + pause2) + (chargeKW/capacityKWh)*(100/60)*(i - (t_charge_step2 + pause2));
