# first try of our opinion formation program
# author: Alexander
...
N = 1000
op = random.normal(N)	#society agent

# Defining the properties of the society
def mu(op1, op2):           # how much should the opinion change?
	return 0.2/(op1-op2)    # this is only taken as example and need to be changed later
...
def SingleAgent(op0, op, u):
	...
	if abs(op1-op0) < u:
		op0 += mu(op0, op1)
		op1 += mu(op1, op0)
	return op0, op1, pos_op1

# Creating the extremists
n = 10      # number of extremists with opinion 0 and 1 splitted symmetrically
ex0 = 0
nex0 = n/2
ex1 = 1
nex1 = n/2
...
for t in xrange(T):
	...
	for i in xrange(n/2):
	    r = random.rand()
	    for j in xrange(p):         # fuck, p und anzahl agenten ist das selbe...
	        if r < k:
