...
%                     neighbour = itr_neighbours.next();
                    ...
                    % repulsive Force
                    F = F + epsilon*(1-distance/(2*ro))^(5/2)*(positionNeighbour-position)/distance;
                    % propulsion
                    F = F + my*(velocityIndividual-velocityNeighbour)*velocityNeighbour/length(velocityNeighbour);
                    % velocity summation
                    sumOverVelocities+= velocityNeighbour
                end
                %Flocking
                F = F + alpha*sunOverVelocities/length(sumOverVelocities);
                %add noise
                F = F + 0;
