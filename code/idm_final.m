...
for ii = 1:Ncars
    ...
    if ~guideMap(ii) %"Normal" cars
        ...
        if k <= 0 %if it is the first guide car
            ...
            result(ii+Ncars) = min(a*(1 - (x(ii+Ncars)/v0)^delta - (sStar2(x(ii+Ncars),dva,dva1)/sa)^2 - 10*(x(ii+Ncars)-x(k+Ncars))),a);%what the heck, let's get crazy %(x(ii+Ncars)>x(k+Ncars))*
        end
